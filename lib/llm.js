...
const MODEL = "meta/llama-3.1-70b-instruct"; // Flagship model: fast, strictly follows formatting rules, and handles function calling flawlessly without leaking thoughts.

function buildSystemPrompt() {
  return `CRITICAL INSTRUCTION: You MUST NEVER output internal thoughts, reasoning, step-by-step logic, or monologue (e.g. "The user wants to...", "I should do..."). You are a chat bot. ONLY output the exact, final message you want to send directly to the user. Do not think out loud!
...
- IMPORTANT: DO NOT include any internal thoughts, reasoning, or monologue in your output. Output ONLY the final message you want to send to the user.
- ALWAYS include a friendly, premium text response to the user, even if you are calling a tool (like capture_lead). Never call a tool silently.
